  ...
  public function __saveRelatedData($model,$value) {
    ...
    foreach ($officeHours as $key => $officeHour) {
      if(($model->state == 'update') && $model->checkRelatedDataExist($this->modelName)){
        $model->getRalatedDataByModelName($this->modelName,true,[['day_id','=',$officeHour['day_id']]])->fill($value)->save();
      }else{
        $officeHourModel = new OfficeHour;
        $officeHourModel->fill($officeHour)->save();
      }
    }
